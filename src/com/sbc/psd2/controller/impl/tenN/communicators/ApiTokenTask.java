...
public class ApiTokenTask implements Runnable {

    @Override
    public void run() {
        LogManager.trace(TenNIdentityManagementCommunicator.class, "getApiToken()");
        String identityManagementCommunicatorEndpoint = AppConfig.getInstance().getIdentityManagementCommunicatorEndPoint();
        ...
        try {
            tokenURL = new URL(identityManagementCommunicatorEndpoint + "/mtls/token");
        ...
        TenNIdentityManagementCommunicator.apiToken = token.getAccess_token();
        TenNIdentityManagementCommunicator.tokenType = token.getToken_type();
    }
}
